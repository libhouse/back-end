...
O projeto *Controllers* promove o desacoplamento entre o processamento das requisições e respostas dos clientes e o *framework* *web* **.NET Core** utilizado na camada de *API*. Essa estratégia viabiliza um reaproveitamento completo do código do projeto em outros *frameworks* da plataforma *.NET*, pois as classes que gerenciam os pedidos de execução das regras de negócio do sistema (**Controllers**) não possuem qualquer vínculo com uma tecnologia específica.
